import os
import json
import requests # ❌ Removed sqlite3
import datetime
import pytz
import uuid
...
from flask import Flask, request, jsonify, send_file # ◀️ এখানে send_file যোগ করা হয়েছে
...
MAIN_BOT_API = f"https://api.telegram.org/bot{MAIN_BOT_TOKEN}" # ◀️ ফিক্স করা হয়েছে
LINK_BOT_API = f"https://api.telegram.org/bot{LINK_BOT_TOKEN}" # ◀️ ফিক্স করা হয়েছে
ADMIN_BOT_API = f"https://api.telegram.org/bot{ADMIN_BOT_TOKEN}" # ◀️ ফিক্স করা হয়েছে
...
def set_webhook(bot_token, path):
    url = f"https://api.telegram.org/bot{bot_token}/setWebhook" # ◀️ ফিক্স করা হয়েছে
...
def post_tweet_to_x(tweet_id, user_id, account_username, tweet_text):
    ...
    try:
        ...
        if response.status_code == 201:
            ...
            if tweet_id_str:
                tweet_link = f"https://x.com/{account_username}/status/{tweet_id_str}"
...
def check_and_post_scheduled_tweets():
    ...
    for i, tweet in enumerate(tweets_to_post, 1):
        ...
        if post_link:
            ...
            try:
                # ✅ FIX: Proper query without "neq.null"
                ...
                if r.status_code == 200:
                    ...
                    for row in link_rows:
                        ...
                        if chat_id:
                            ...
                            # ✅ CRITICAL: শুধু link পাঠান, কোন extra text নয়
